        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 pt-2">
          <div className="space-y-3 p-4 bg-primary/5 rounded-[2rem] border border-primary/10 shadow-[inset_0_1px_4px_rgba(0,0,0,0.02)]">
            <label className="text-xs font-black flex items-center gap-2 text-primary uppercase tracking-widest">
              <CalendarIcon className="h-4 w-4" /> 1. Dia da Semana
            </label>
            <Tabs 
              value={entry.dayOfWeek} 
              onValueChange={(v) => {
                const newDay = v as any;
                if (entry.visitDate) {
                  const dayMap: Record<number, any> = { 0: 'domingo', 1: 'segunda', 5: 'sexta', 6: 'sabado' };
                  const currentDataDay = dayMap[entry.visitDate.getDay()];
                  if (currentDataDay !== newDay) {
                    onUpdateEntry({ dayOfWeek: newDay, visitDate: null });
                  } else {
                    onUpdateEntry({ dayOfWeek: newDay });
                  }
                } else {
                  onUpdateEntry({ dayOfWeek: newDay });
                }
              ...
              className="w-full"
            >
              <TabsList className="grid grid-cols-4 w-full h-auto p-1 bg-white/50 rounded-2xl border border-primary/5">
                {['sexta', 'sabado', 'domingo', 'segunda'].map(day => (
                  <TabsTrigger 
                    key={day} 
                    value={day} 
                    className="py-2.5 text-[13px] font-black uppercase rounded-xl data-[state=active]:bg-primary data-[state=active]:text-white data-[state=active]:shadow-lg transition-all"
                  >
                    {day === 'sabado' ? 'Sáb' : day === 'domingo' ? 'Dom' : day === 'segunda' ? 'Seg' : 'Sex'}
                  </TabsTrigger>
                ))}
              </TabsList>
            </Tabs>
          ...
          <div className="space-y-3 p-4 bg-primary/5 rounded-[2rem] border border-primary/10 shadow-[inset_0_1px_4px_rgba(0,0,0,0.02)]">
            <label className="text-xs font-black flex items-center gap-2 text-primary uppercase tracking-widest">
              <CalendarIcon className="h-4 w-4" /> 2. Data no Calendário
            </label>
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className={cn("w-full h-11 justify-start text-left font-bold text-xs rounded-2xl border-white/80 bg-white/70 backdrop-blur-sm shadow-sm hover:bg-white hover:border-primary/30 hover:text-foreground transition-all", !entry.visitDate && "text-muted-foreground")}>
                  ...
                  {entry.visitDate ? format(entry.visitDate, "dd/MM/yyyy (EEEE)", { locale: ptBR }) : "Escolher data..."}
                </Button>
              </PopoverTrigger>
