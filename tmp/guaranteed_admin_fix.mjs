...
// 2. Guaranteed replacement of handleCreateInternalBooking
const startKey = 'const handleCreateInternalBooking = async () => {';
const endPoint1 = 'setLoading(false);';
const endPoint2 = '};';

let startIdx = data.indexOf(startKey);
if (startIdx !== -1) {
    let searchFrom = startIdx + startKey.length;
    let end1 = data.indexOf(endPoint1, searchFrom);
    if (end1 !== -1) {
        let end2 = data.indexOf(endPoint2, end1 + endPoint1.length);
        if (end2 !== -1) {
            const finalEnd = end2 + endPoint2.length;
    ...
    try {
      ...
      if (!name || !visit_date || !isValidCPF(cpf)) {
        toast({ title: !isValidCPF(cpf) ? "CPF INVÁLIDO" : "Campos obrigatórios", description: "O CPF deve ser válido para evitar erros no Asaas.", variant: "destructive" });
...
// 3. UI Grid
data = data.replace('grid-cols-1 md:grid-cols-3', 'grid-cols-1 md:grid-cols-4');

// 4. Injetar campo CPF
if (!data.includes('CPF (Validação)')) {
    const endName = data.indexOf('</div>', data.indexOf('placeholder="Nome Completo"')) + 6;
    const cpfField = `
    ...
    data = data.substring(0, endName) + cpfField + data.substring(endName);
}
...
console.log("GUARANTEED SYNC DONE.");
