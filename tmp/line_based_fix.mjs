...
// 1. Add Import at Line 1 (0-indexed)
if (!lines[1] || !lines[1].includes('isValidCPF')) {
    lines.splice(1, 0, "import { isValidCPF } from '@/utils/cpf-validator';");
}

// 2. Add State (search for isNewBookingOpen)
const iIdx = lines.findIndex(l => l.includes('const [isNewBookingOpen'));
if (iIdx !== -1 && !lines.some(l => l.includes('generatedPix'))) {
    lines.splice(iIdx + 1, 0, "  const [generatedPix, setGeneratedPix] = useState<any>(null);");
}

// 3. Add cpf to newBookingData
const nIdx = lines.findIndex(l => l.includes("name: '',"));
if (nIdx !== -1 && !lines.some(l => l.includes("cpf: '',"))) {
    lines.splice(nIdx + 1, 0, "    cpf: '',");
}

// 4. REPLACE handleCreateInternalBooking (Lines 474-578)
// Lines in view_file were 1-indexed. 474 is index 473.
// We'll search exactly for the start and end to be safe.
...
if (fStart !== -1 && fEnd !== -1) {
    const newFuncLines = [
        ...
        '        toast({ title: !isValidCPF(cpf) ? "CPF INVÁLIDO" : "Campos obrigatórios", description: "O CPF deve ser válido para prosseguir.", variant: "destructive" });',
    ...
    lines.splice(fStart, fEnd - fStart + 1, ...newFuncLines);
}

// 5. Update Grid
const gIdx = lines.findIndex(l => l.includes('grid grid-cols-1 md:grid-cols-3 gap-6'));
if (gIdx !== -1) lines[gIdx] = lines[gIdx].replace('md:grid-cols-3', 'md:grid-cols-4');

// 6. Inject CPF Input (After Phone)
const pIdx = lines.findIndex(l => l.includes('placeholder="DDD + Número"'));
const divEndIdx = lines.findIndex((l, i) => i > pIdx && l.includes('</div>'));
if (divEndIdx !== -1 && !lines.some(l => l.includes('Obrigatório)'))) {
...
if (sStart !== -1 && !lines.some(l => l.includes('generatedPix ?'))) {
    const line = lines[sStart];
    lines[sStart] = `                        {generatedPix ? (
    ...
    // Find the end before DialogFooter
    const footerIdx = lines.findIndex(l => l.includes('</DialogFooter>'));
    if (footerIdx !== -1) {
        lines.splice(footerIdx, 0, '                    )}');
    }
}

fs.writeFileSync(path, lines.join('\n'));
console.log("LINE-BASED SYNC SUCCESS.");
