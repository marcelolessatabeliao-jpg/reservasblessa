...
async function checkUpdate() {
  const { data: bks } = await supabase.from('bookings').select('id, status').limit(1);
  if (bks && bks.length > 0) {
    const originalStatus = bks[0].status;
    console.log("Found legacy booking:", bks[0].id, "status:", originalStatus);
    const { data: upd, error } = await supabase.from('bookings').update({ status: 'cancelled' }).eq('id', bks[0].id).select();
